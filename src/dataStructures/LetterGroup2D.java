...
public class LetterGroup2D {
    ...
    public void sortByFrequency() {
        ...
        for (int c = 0; c < 26; c++) {  //  FOR every column from index of 2 until index of 26 (our UNSORTED range of letters)

            for(int r = 0; r < array2D.length; r++) {                      //  FOR each row in that column
                array2D[r][array2D[r].length-1] = array2D[r][c];     //  COPY the values of that column to column 28 (which is our temporary location)
            }
            int index = c - 1;                                                                    //  The index position of the last data element known to be sorted correctly
            while (index >= 0 && array2D[1][index] < array2D[1][array2D[1].length-1]) {  //  WHILE sortCorrectly index >=0 AND check that the value at that index is LESS THAN the row to sorted in column 28
